...
function DrawEntities.Install(BattleView, imgHandles)
    ...
    function BattleView:DrawEnemies(nvg, l, bs)
        ...
        if MAX_DRAW_ENEMIES then
            for _, e in ipairs(bs.enemies) do
                if not e.dead and e.isBoss and drawnCount < MAX_DRAW_ENEMIES then
                    e._fxDraw = true
                    drawnCount = drawnCount + 1
                end
            end
            for _, e in ipairs(bs.enemies) do
                if not e.dead and not e.isBoss then
        ...
        for _, e in ipairs(bs.enemies) do
            if not e.dead and (not MAX_DRAW_ENEMIES or e._fxDraw) then
                local sx = l.x + e.x
                local sy = l.y + e.y
                -- 视口外剔除
    ...
    function BattleView:DrawPlayer(nvg, l, bs)
        ...
        if GameState.iceArmorActive then
