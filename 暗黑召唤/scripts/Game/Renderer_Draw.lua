...
function Renderer.DrawEnemies(vg)
    for _, e in ipairs(State.enemies) do
        if e.alive then
            ...
            if e.isBoss then
                local nx, ny = Grid.GetPathOutwardNormal(e.progress, Renderer.gridOffsetX, Renderer.gridOffsetY)
            ...
            local pdx, pdy = Grid.GetPathDirection(e.progress, Renderer.gridOffsetX, Renderer.gridOffsetY)
            local flipX = (pdx < -0.1) or (pdy < -0.1)

            -- 敌人身体：优先使用图片，无图片时回退到矢量形状
            local mobIcon = e.typeDef.icon
            local mobImg = mobIcon and EnsureMobImage(vg, mobIcon) or -1
            if mobImg > 0 then
                -- 图片绘制：BOSS 放大，普通怪物按 size 缩放
                local imgSize = e.isBoss and (size * 2.8) or (size * 2.5)
                DrawMobImage(vg, mobImg, drawX, drawY, imgSize, bodyAlpha, flipX)
            else
                DrawEnemyShape(vg, drawX, drawY, size, shape, c[1], c[2], c[3], bodyAlpha)
            end
            ...
            if e.hitFlash and e.hitFlash > 0 then
                ...
                if mobImg > 0 then
                    -- 图片模式：叠加混合重绘图片，使像素变亮变白
                    local imgSize = e.isBoss and (size * 2.8) or (size * 2.5)
                    nvgSave(vg)
                    nvgGlobalCompositeOperation(vg, NVG_LIGHTER)
                    DrawMobImage(vg, mobImg, drawX, drawY, imgSize, flashAlpha, flipX)
                    nvgRestore(vg)
                else
                    DrawEnemyShape(vg, drawX, drawY, size, shape, 255, 255, 255, flashAlpha)
                end
            end
...
function Renderer.DrawEnemyCount(vg, ox, oy)
    ...
    local labelY = topY - Config.CELL_SIZE * 0.55 -- 路径上方
    ...
    if State.overloading then
        ...
        nvgText(vg, centerX, labelY - 16, string.format("超限! %.1fs", remain), nil)
    end

    do
        local countText = count .. "/" .. max
        ...
        nvgText(vg, centerX, labelY, "\u{1F47E} " .. countText, nil)
        ...
        if State.phase == State.PHASE_PLAYING
           and State.currentWave < Config.WAVES_PER_STAGE then
            ...
            nvgText(vg, centerX, labelY + 16, string.format("下一波 %ds", math.ceil(nextWaveIn)), nil)
        end
    end
end
...
function Renderer.DrawBossBar(vg, w)
    ...
    -- 倒计时（右侧）
    local timerX = hpAreaX + hpAreaW + 8
    local remain = math.max(0, State.bossTimer)
    local timeStr = string.format("%02d:%02d", math.floor(remain / 60), math.floor(remain % 60))
    ...
    nvgText(vg, timerX, barY + barH * 0.3, "BOSS", nil)
    ...
    nvgText(vg, timerX + 30, barY + barH * 0.3, timeStr, nil)

    -- 剩余时间进度条（小的）
    local timerBarX = timerX
    local timerBarW = barW - hpAreaW - 20
    local timerBarY = barY + barH * 0.65
    local timerBarH = 3
    local timerRatio = remain / Config.BOSS_TIMER_MAX
    ...
    if timerRatio > 0 then
        ...
        nvgFillColor(vg, nvgRGBA(tr, tg, tb, 200))
        nvgFill(vg)
    end
end
