            ...
            UI.Panel {
                ...
                children = {
                    UI.Button {
                        ...
                        borderRadius = 8,
                        textAlign = "center",
                        variant = anyCanClaim and "primary" or "outline",
                        disabled = not anyCanClaim,
                        onClick = function()
                            local claimed = false
                            for idx = 1, #milestones do
                                local ok = LaunchGiftData.ClaimMilestone(idx)
                                if ok then claimed = true end
                            end
                            if claimed then
                                Toast.Show("里程碑奖励已领取", S.textGreen)
                                LaunchGiftUI.Refresh()
                            else
    ...
    return UI.Panel {
        ...
        children = {
            ...
            UI.Button {
                ...
                disabled = claimed,
                onClick = function()
                    local ok, msg = LaunchGiftData.ClaimDailyPulls()
