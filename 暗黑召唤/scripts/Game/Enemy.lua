...
local function CalcScalesFromGlobalWave(globalWave)
    ...
    local stageScale = (Config.STAGE_HP_BASE + Config.STAGE_HP_PER_STAGE * (stageNum - 1))
                       * (Config.STAGE_HP_EXPO ^ (stageNum - 1))
...
function Enemy.CreateEnemyFromDef(typeDef, waveNum, hpScale, speedScale, isElite, affixes)
    ...
    print(string.format("[Enemy] Spawned %s%s%s HP=%d SPD=%.0f",
        prefix, typeDef.name, affixNames, math.floor(enemy.hp), enemy.speed))
...
function Enemy.CreateBoss(bossDef, waveNum, hpScale, speedScale, affixes, tier)
    ...
    print(string.format("[Enemy] BOSS Spawned: %s (Tier %d)%s HP=%d",
        bossDef.name, tier, affixNames, math.floor(enemy.hp)))
...
function Enemy.TakeDamage(enemy, damage)
    ...
    if dodgeChance > 0 and math.random() < dodgeChance then
        State.floatingTexts[#State.floatingTexts + 1] = {
            ...
            y = enemy.y - enemy.typeDef.size - 5,
            life = 0.6,
    ...
    if enemy.firstHitArmored then
        ...
        State.floatingTexts[#State.floatingTexts + 1] = {
            text = "护甲",
            x = enemy.x, y = enemy.y - enemy.typeDef.size - 5,
            life = 0.5,
        ...
        if enemy.typeDef.specialPassive == "death_silence" then
            ...
            for _, tower in ipairs(State.towers) do
                ...
                if dx * dx + dy * dy <= range * range then
                    tower.silenceTimer = math.max(tower.silenceTimer or 0, dur)
                end
            end
            State.floatingTexts[#State.floatingTexts + 1] = {
                text = "沉默!",
...
function Enemy.ApplySlow(enemy, duration, rate)
    -- 免疫控制检查
    if enemy.typeDef.passive == "immune_cc" then return end
    if enemy.affixIds["void_aura"] then return end
...
function Enemy.ApplyDOT(enemy, damage, duration)
    ...
    -- dot_immune 坦克被动
    if enemy.typeDef.dotImmune then return end
...
local function UpdateAuras(gridOffsetX, gridOffsetY)
    ...
    -- 遍历所有光环怪，施加范围效果
    for _, src in ipairs(State.enemies) do
        if src.alive and src.typeDef.passive == "aura" and src.typeDef.aura then
            local aura = src.typeDef.aura
            local range = aura.range or 60
            local rangeSq = range * range

            for _, tgt in ipairs(State.enemies) do
            ...
            if e.typeDef.passive == "phase" then
                if e.phaseActive then
                    ...
                    if e.phaseTimer <= 0 then
                        e.phaseActive = false
                        e.phaseTimer = e.typeDef.phaseInterval or 5.0
                    end
                else
                    e.phaseTimer = e.phaseTimer - dt
                    if e.phaseTimer <= 0 then
                        e.phaseActive = true
                        e.phaseTimer = e.typeDef.phaseDuration or 2.5
                    end
                end
            end
