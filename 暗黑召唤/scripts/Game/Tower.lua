...
function Tower.RefreshAllStats()
    for _, tower in ipairs(State.towers) do
        ...
        if tower.isLeader then
            tower.attack = heroStats.atk + equipAtk
            tower.range = typeDef.baseRange + levelRange
            tower.speed = typeDef.baseSpeed / (1 + (heroStats.spdBonus or 0))
        else
            tower.attack = heroStats.atk * starMult + equipAtk
            tower.range = typeDef.baseRange + starRange + levelRange
            tower.speed = typeDef.baseSpeed / starSpeed / (1 + (heroStats.spdBonus or 0))
        end
        ...
        tower.armorPen = heroStats.armorPen or 0
        tower.critRate = heroStats.critRate or 0
