...
local function BuildPrivilegeContent()
    ...
    if not unlocked and card.adsRequired > 0 then
        ...
        lockChildren[#lockChildren + 1] = ctx.UI.Panel {
            ...
            children = {
                ctx.UI.Panel { width = 22, height = 22, backgroundImage = "image/icon_lock_20260408182729.png", backgroundFit = "contain" },
        ...
        if dailyLimitReached then
            ...
            lockChildren[#lockChildren + 1] = ctx.UI.Panel {
                ...
                alignItems = "center",
                children = {
                    ctx.UI.Panel { width = 18, height = 18, backgroundImage = "image/icon_watch_ad_20260408182809.png", backgroundFit = "contain", marginRight = 4, opacity = 0.4 },
        ...
        elseif canWatch then
            lockChildren[#lockChildren + 1] = ctx.UI.Panel {
                ...
                children = {
                    ctx.UI.Panel { width = 18, height = 18, backgroundImage = "image/icon_watch_ad_20260408182809.png", backgroundFit = "contain", marginRight = 4 },
    ...
    local cardPanel = ctx.UI.Panel {
        width = "100%",
        flex = hasInfo and 6 or 9,
        position = "relative",
        backgroundColor = { 25, 20, 45, 220 },
        backgroundImage = "image/privilege_card_bg_20260408093708.png",
    ...
    if lockOverlay then
        -- 未解锁：锁图标 + 广告进度 + 观看按钮
        cardPanel:AddChild(ctx.UI.Panel {
            position = "absolute",
            top = "15%",
            left = 0, right = 0,
            alignItems = "center",
        ...
        if remainLabel then
            nameChildren[#nameChildren + 1] = remainLabel
        end
        cardPanel:AddChild(ctx.UI.Panel {
            position = "absolute",
            top = "15%",
    ...
    -- 用固定高度的外层容器，让 subTabBar(flex=1) 和 cardPanel(flex=9) 按比例分配
    -- ScrollView 内部 flex 子元素需要明确的父高度才能生效
    local screenH = graphics:GetHeight() / graphics:GetDPR()
    local wrapper = ctx.UI.Panel {
        width = "100%",
        height = screenH * 0.72,
        children = hasInfo
            and { subTabBar, infoPanel, cardPanel }
            or  { subTabBar, cardPanel },
    }
