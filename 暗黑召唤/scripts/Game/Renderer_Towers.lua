...
local function DrawTowerIcon(vg, icon, x, y, size, color, star, alpha, towerRef)
    ...
    if SpriteSheet.Has(icon) then
        ...
        if star and star > 0 then
            ...
            nvgText(vg, x, y + drawSize * 0.5 + 2, string.rep("\u{2605}", star), nil)
        end
        return
    end
    ...
    if star and star > 0 then
        ...
        nvgFillColor(vg, nvgRGBA(
            Config.COLORS.starColor[1], Config.COLORS.starColor[2],
            Config.COLORS.starColor[3], alpha))
        local starStr = string.rep("\u{2605}", star)
...
function Renderer.DrawTowers(vg, ox, oy)
    for _, tower in ipairs(State.towers) do
        ...
        -- 精灵图角色待机漂浮动画（正弦波上下缓动，幅度3px，周期2秒）
        local hasSprite = SpriteSheet.Has(tower.typeDef.icon)
        if hasSprite and not (State.dragging and State.dragTower and State.dragTower.id == tower.id) then
            cy = cy + math.sin(ctx.spriteFloatTime * math.pi) * 3
        end
        ...
        -- 底部浮动阴影（椭圆，随漂浮动画缩放）
        local floatPhase = math.sin(ctx.spriteFloatTime * math.pi)  -- -1~1
