...
local function CreateDealRow(label, desc, iconId, claimed, canClaim, onClaim)
    ...
    if iconId then
        ...
        iconWidget = ctx.UI.Panel { width = 28, height = 28, backgroundImage = "image/icon_free_gift_20260408182701.png", backgroundFit = "contain" }
    end
    ...
    elseif canClaim then
        btnChildren = {
            ctx.UI.Panel { width = 16, height = 16, backgroundImage = "image/icon_watch_ad_20260408182809.png", backgroundFit = "contain", marginRight = 4 },
            ctx.UI.Label { text = "观看", fontSize = 13, fontColor = { 255, 255, 255 }, fontWeight = "bold" },
        }
        btnBg = { 100, 70, 180, 255 }
        btnBorder = { 180, 140, 255, 200 }
    else
        btnChildren = { ctx.UI.Panel { width = 20, height = 20, backgroundImage = "image/icon_lock_20260408182729.png", backgroundFit = "contain" } }
        btnBg = { 45, 40, 55, 200 }
    ...
    children[#children + 1] = ctx.UI.Panel {
        ...
        children = {
            ...
            ctx.UI.Panel {
                ...
                marginRight = 10,
                children = {
                    ctx.UI.Panel { width = 28, height = 28, backgroundImage = "image/icon_free_gift_20260408182701.png", backgroundFit = "contain" },
                },
            },
            -- 文字 + 奖励图标
            ctx.UI.Panel {
                ...
                ctx.UI.Panel {
                    ...
                    onClick = (canClaim) and function()
                        ...
                        if not sdk or not sdk.ShowRewardVideoAd then
                            Toast.Show("广告不可用", { 255, 100, 80 })
                            return
                        end
                        ---@diagnostic disable-next-line: undefined-global
                        sdk:ShowRewardVideoAd(function(success)
                            if not success then
                                Toast.Show("广告未完成", { 255, 100, 80 })
                                return
                            end
                            AdTracker.Record()
