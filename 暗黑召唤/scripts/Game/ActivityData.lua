...
function ActivityData.InitDefault()
    return {
        signIn = {
            ...
            claimedCount = 0,
            lastLoginDate = "",
            cycleStart = TodayStr(),  -- 本轮30天周期开始日期
        },
    }
end
...
function ActivityData.Load()
    ...
    if saved and saved.signIn and saved.signIn.totalLogins ~= nil then
        ...
        si.cycleStart = si.cycleStart or TodayStr()

        -- 兼容旧数据：如果存在 month 字段但没有 cycleStart，迁移
        if si.month and not saved.signIn.cycleStart then
            si.cycleStart = TodayStr()
            si.month = nil
        end

        -- 30天周期重置：距离周期开始已过30天
        local elapsed = DaysSince(si.cycleStart)
        if elapsed >= CYCLE_DAYS then
            print("[ActivityData] Cycle expired (" .. elapsed .. " days since " .. si.cycleStart .. "), resetting sign-in")
            si.totalLogins = 0
            si.claimedCount = 0
            si.lastLoginDate = ""
            si.cycleStart = TodayStr()
